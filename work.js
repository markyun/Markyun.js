...
//判断节点是否存在
  if ($("input[name='img_ids']").length>0) {  //判断节点是否存在
...
$('.l_nav a').each(function() {
    ...
    //13———————————————————
    //找到父类的父类然后向下查找.intro的:first个。操作
    ...
    //	* 包含
    //	! 不等于
    $("input[name^='list']").live("blur", function() {
        console.log($(this));
    });
    $(function() {
        var t = $ {
    ...
    //15———————————————————
    //ajaxFileUpload图片上传、限制后缀和张数

    HTML： 
        <form action="id_auth2.jsp" method="post" enctype="multipart/form-data"> 
...
function is_vnum(node) {
    var bol=false;
    $.ajax({url:"authvnum.jsp",type:"post",async:false,dataType:"json",success:function(rs){
//       rs = -1;
            if (rs == -1) {
                check();
...
//27———————2个节点（“li”）交换位置——— 
    $("body").delegate('.nxt', 'click', function() {
